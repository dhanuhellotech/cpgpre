...
const sendContactEmail = async (formData) => {
  try {
    ...
    const response = await axios.post('http://localhost:5678/api/contact', formData);
...
export default function Contact() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const response = await axios.post('http://localhost:5678/contacts', formData);
  ...
  const fetchDefaultAddress = async () => {
    ...
    try {
      const response = await axios.get('http://localhost:5678/api/default-address');
      setAddresses([response.data]);
    } catch (error) {
